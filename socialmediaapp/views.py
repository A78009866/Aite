...
from .models import Notification, Post, Like, Comment, CustomUser
...
def chat_list(request, username):
    ...
    user_data = []
    for user in users:
        # جلب آخر رسالة من المستخدم الحالي إلى المستخدم الآخر
        last_sent_message = Message.objects.filter(
            sender=current_user,
            receiver=user
        ).order_by('-timestamp').first()
        
        # جلب آخر رسالة من المستخدم الآخر إلى المستخدم الحالي
        last_received_message = Message.objects.filter(
            sender=user,
    ...
    user_data.sort(key=lambda x: x['last_time'] if x['last_time'] else datetime.datetime.min.replace(tzinfo=pytz.UTC), reverse=True)

    return render(request, 'chat_list.html', {'all_users': user_data, 'username': username})
